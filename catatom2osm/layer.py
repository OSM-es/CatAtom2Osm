...
class BaseLayer(QgsVectorLayer):
    ...
    def to_osm(self, tags_translation=translate.all_tags, data=None, tags={}, 
        ...
        """
        if data is None:
            data = osm.Osm(upload, generator=config.app_name + ' ' + config.app_version)
...
class PolygonLayer(BaseLayer):
    ...
    def __init__(self, path, baseName, providerLib = "ogr"):
        ...
        self.dup_thr = config.dup_thr # Distance in meters to merge nearest vertex.
        self.cath_thr = config.dist_thr # Threshold in meters for cathetus reduction
        self.straight_thr = config.straight_thr # Threshold in degrees from straight angle to delete a vertex
        self.dist_thr = config.dist_thr # Threshold for topological points.
