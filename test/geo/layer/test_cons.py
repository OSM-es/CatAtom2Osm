...
class TestConsLayer(unittest.TestCase):
    ...
    def test_merge_adjacent_features(self):
        parts = [p for p in
                 self.layer.search("localId like '8840501CS5284S_part%%'")]
    ...
    def test_remove_parts_below_ground(self):
        to_clean = [f.id() for f in
                    self.layer.search('lev_above=0 and lev_below>0')]
    ...
    def test_index_of_parts(self):
        ...
        p = {f.id(): f for f in self.layer.getFeatures() if
             self.layer.is_part(f)}
    ...
    def test_index_of_building_and_parts(self):
        ...
        self.assertTrue(all([localid == bu['localid']
                             for (localid, group) in list(buildings.items()) for
                             bu in group]))
        self.assertTrue(all([localid == pa['localid'][0:14]
                             for (localid, group) in list(parts.items()) for pa
                             in group]))
    ...
    def test_get_parts(self):
        ...
        for outline in self.layer.search("localId = '8840501CS5284S'"):
            ...
            self.assertEqual(len(parts_inside), sum([len(p) for p in list(
                parts_for_level.values())]))
            for part in parts_inside:
                self.assertIn(part, parts_for_level[
                    (part['lev_above'], part['lev_below'])])
    ...
    def test_merge_adjacent_parts(self, ref=None):
        ...
        for outline in self.layer.search("localId = '%s'" % ref):
            ...
            if len(parts_for_level) > 1:
                ...
                self.assertEqual(set(cn), set([p.id() for p in parts_for_level[
                    max_level, min_level]]))
            self.assertEqual(max([l[0] for l in list(parts_for_level.keys())]),
                             max_level)
            self.assertEqual(max([l[1] for l in list(parts_for_level.keys())]),
                             min_level)
    ...
    def test_delete_invalid_geometries(self):
        ...
        self.assertEqual(r,
                         [(round(p.x(), 2), round(p.y(), 2)) for p in mp[0][0]])
        request = QgsFeatureRequest().setFilterFid(
            self.layer.featureCount() - 1)
        f = next(self.layer.getFeatures(request))
        mp = Geometry.get_multipolygon(f)
    ...
    def test_conflate(self):
        ...
        d = osm.Osm()
        d.Way(((-16.44211325828, 28.23715394977),
        ...
        d.Way(((-16.44016295806, 28.23657619128),
        ...
        d.Way(
            ((-16.44051231511, 28.23655551417), (-16.44042112, 28.23650529975),
