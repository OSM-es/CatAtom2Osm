...
class OsmxmlTest(unittest.TestCase):
    def test_serialize(self):
        ...
        mp1 = [(0, 0), (10, 0), (10, 6), (0, 6), (0, 0)]
        mp2 = [(8, 1), (9, 1), (9, 2), (8, 2), (8, 1)]
        r = data.MultiPolygon([[mp1, mp2]])
    ...
    def test_deserialize(self):
        ...
        for (i, node) in enumerate([(12, 0), (14, 0), (14, 2), (12, 2), (12, 0)]):
            nodexml = etree.Element(
                "node", dict(id=str(-i - 10), lon=str(node[0]), lat=str(node[1]))
            )
            root.append(nodexml)
        ...
        for (i, node) in enumerate([(0, 0), (10, 0), (10, 6), (0, 6), (0, 0)]):
            nodexml = etree.Element(
                "node", dict(id=str(-i - 20), lon=str(node[0]), lat=str(node[1]))
            )
            root.append(nodexml)
            wayxml.append(etree.Element("nd", dict(ref=str(nodexml.get("id")))))
        root.append(wayxml)
        ...
        for (i, node) in enumerate([(8, 1), (9, 1), (9, 2), (8, 2), (8, 1)]):
            nodexml = etree.Element(
                "node", dict(id=str(-i - 30), lon=str(node[0]), lat=str(node[1]))
            )
            root.append(nodexml)
            wayxml.append(etree.Element("nd", dict(ref=str(nodexml.get("id")))))
        root.append(wayxml)
        ...
        relxml.append(
            etree.Element("member", dict(type="way", ref="-101", role="outter"))
        )
        relxml.append(
            etree.Element("member", dict(type="way", ref="-102", role="inner"))
        )
        ...
        relxml.append(
            etree.Element("member", dict(type="way", ref="-101", role="dummy"))
        )
        root.append(relxml)
        ...
        relxml.append(
            etree.Element("member", dict(type="way", ref="-199", role="dummy"))
        )
        root.append(relxml)
        fo = BytesIO(etree.tostring(root))
        result = osmxml.deserialize(fo, result)
